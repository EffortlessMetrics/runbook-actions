...
public sealed class DaemonClient : IAsyncDisposable
{
    ...
    private async Task MaintainConnectionAsync()
    {
        ...
        while (_cts is { IsCancellationRequested: false })
        {
            try
            {
                ...
                // Hello handshake.
                await SendRawAsync(OutboundMessageFactory.Hello(ClientId));

                // Wait for hello_ack (timeout 5s).
                var ackJson = await ReceiveOneMessageAsync(TimeSpan.FromSeconds(5));
                if (ackJson is not null && DaemonProtocol.IsHelloAckProtocolMismatch(ackJson, out var detail))
                {
    ...
    public Task SendKeypadPressAsync(int slot)
        => SendRawAsync(OutboundMessageFactory.KeypadPress(slot));

    public Task SendDialpadButtonPressAsync(string button)
        => SendRawAsync(OutboundMessageFactory.DialpadButtonPress(button));

    public Task SendPageAsync(string direction)
        => SendRawAsync(OutboundMessageFactory.Page(direction));
    ...
    public Task SendAdjustmentAsync(string kind, int delta)
        => SendRawAsync(OutboundMessageFactory.Adjustment(kind, delta));
    ...
    private void FlushCoalescedAdjustments(object? _)
    {
        ...
        if (roller != 0) _ = SendRawAsync(OutboundMessageFactory.Adjustment("roller", roller));
        if (dial != 0) _ = SendRawAsync(OutboundMessageFactory.Adjustment("dial", dial));
    }
